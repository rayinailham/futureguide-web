...
onMounted(() => {
  ...
  ctx = gsap.context(() => {
    ...
    // Parallax for remaining textures (skew-grid + halftone)
...
<template>
  ...
  >
    <!-- Minimal textures: skewed grid + halftone only — desktop only -->
    ...
    <div
      ...
      style="min-height: 100dvh"
    >
      <!-- Two-column hero: copy left, stacked radar cards + meta right -->
      <div class="grid gap-10 lg:grid-cols-12 lg:items-start lg:gap-12">
        <!-- LEFT: copy column (compact) -->
        <div class="flex flex-col gap-5 sm:gap-6 lg:col-span-7">
          <!-- Eyebrow -->
          ...
          <div class="sm:hidden h-px w-full bg-[var(--color-ink-100)]"></div>

          <!-- Headline (split lines, asymmetric editorial) — scaled down for 2-col layout -->
          <h1
            class="display max-w-[16ch] text-[clamp(2rem,5.6vw,4.75rem)] text-[var(--color-ink-900)]"
          >
          ...
          <!-- Sub paragraph (mobile + desktop variants) -->
          <p
            data-sub
            class="sm:hidden max-w-[42ch] text-balance text-sm leading-relaxed text-[var(--color-ink-600)]"
          >
          ...
          <p
            data-sub
            class="hidden sm:block max-w-[52ch] text-balance text-sm leading-relaxed text-[var(--color-ink-600)] sm:text-base"
          >
        ...
        <div class="relative flex flex-col gap-6 sm:gap-8 lg:col-span-5">
          ...
          <p
            data-cta
            class="font-mono text-[10px] uppercase tracking-[0.22em] text-[var(--color-ink-500)] lg:text-right"
          >
          ...
          <!-- Stats meta -->
          <ul
            class="grid w-full grid-cols-2 gap-px overflow-hidden rounded-xl sm:rounded-2xl border hairline bg-[color-mix(in_oklab,_var(--color-ink-900)_5%,transparent)] sm:grid-cols-4"
          >
            <li
              v-for="s in stats"
              :key="s.k"
              data-meta
              class="flex h-20 flex-col justify-between bg-[var(--color-paper)] p-3 sm:h-24 sm:p-3.5"
            >
              ...
              <span class="display text-2xl text-[var(--color-ink-900)] leading-none sm:text-2xl">{{ s.k }}</span>
            </li>
          </ul>
        </div>
      </div>
